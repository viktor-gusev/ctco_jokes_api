...
export class JokesService {
  public async retrieveJokes(
    ...
    if (category && category.length > 0) {
      searchCategories = category.filter(
        el => categories.indexOf(el.toLowerCase()) >= 0
      );
    } else {
  ...
  public async updateCategory(id, isBanned): Promise<void> {
    ...
    if (!entity) throw new Error("Category was not found");
  ...
  public async updateFlag(id, isActive): Promise<void> {
    ...
    if (!entity) throw new Error("Flag was not found");
  ...
  private async getCategories(): Promise<string[]> {
    const categories: Category[] = await this.searchCategories({
      isBanned: false
    });
